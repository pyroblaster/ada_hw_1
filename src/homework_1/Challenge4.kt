...
fun main() {
    ...
    val average = data.flatMap { it.value }
            .filter { it in 0..Int.MAX_VALUE}
            .asSequence()
            .average()
    println(average)

    val faulty = data.filterValues { it.contains(-1) }
            .keys.toString()
            .removeSurrounding("[", "]")
    println(faulty)

    val negative= data.flatMap { it.value }
            .filter { it in -Int.MAX_VALUE..0 }
            .asSequence()
            .count()
    println(negative)
}
